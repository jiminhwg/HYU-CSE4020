...
def generate_curve_points(control_points):
    ...
    # Quadratic Bézier basis matrix
    M = np.array([[1, -2, 1],
                  [-2, 2, 0],
                  [1, 0, 0]], dtype=np.float32)

    # Geometry matrix (3x3: each row is a vec3 [x, y, z])
    P = np.array([
        [P0.x, P0.y, P0.z],
        [P1.x, P1.y, P1.z],
        [P2.x, P2.y, P2.z]
    ...
    for t in np.linspace(0, 1, 101):
        T = np.array([t**2, t, 1], dtype=np.float32)  # 1x3

        p = T @ M @ P  # result is a 1x3 vector (x, y, z)
...
def main():
    ...
    window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, '2023049998', None, None)
    if not window:
        glfwTerminate()
        return
