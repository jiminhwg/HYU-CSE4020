...
def key_callback(window, key, scancode, action, mods):
    global g_cam_ang, g_cam_height,x_ang, z1_ang, z2_ang
    if key==GLFW_KEY_ESCAPE and action==GLFW_PRESS:
        glfwSetWindowShouldClose(window, GLFW_TRUE);
    else:
        if action==GLFW_PRESS or action==GLFW_REPEAT:
            if key == GLFW_KEY_A:  # Z counterclockwise
                z1_ang += np.radians(10)
            elif key == GLFW_KEY_Z:  # Z clockwise
                z1_ang -= np.radians(10)
            elif key == GLFW_KEY_S:  # X counterclockwise
                x_ang += np.radians(10)
            elif key == GLFW_KEY_X:  # X clockwise
                x_ang -= np.radians(10)
            elif key == GLFW_KEY_D:  # Z counterclockwise
                z2_ang += np.radians(10)
            elif key == GLFW_KEY_C:  # Z clockwise
                z2_ang -= np.radians(10)
            elif key == GLFW_KEY_V:  # Reset
                z1_ang = x_ang = z2_ang = 0.0
...
def main():
    ...
    while not glfwWindowShouldClose(window):
        ...
        Rx = glm.rotate(x_ang, (1, 0, 0))  # rotation around X-axis
        Rz1 = glm.rotate(z1_ang, (0, 0, 1))  # rotation around Z-axis first
        Rz2 = glm.rotate(z2_ang, (0, 0, 1))  # rotation around Z-axis second
