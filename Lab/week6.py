...
def framebuffer_size_callback(window, width, height):
    ...
    g_P = glm.perspective(glm.radians(45.0), aspect_ratio, 0.1, 100.0)
...
def main():
    ...
    initial_width, initial_height = glfwGetFramebufferSize(window)
    aspect = initial_width / initial_height  
    g_P = glm.perspective(glm.radians(45.0), aspect, 0.1, 100.0)
    ...
    while not glfwWindowShouldClose(window):
        ...
        cam_pos = glm.vec3(5*np.sin(g_cam_ang), g_cam_height + 2, 5*np.cos(g_cam_ang))
        V = glm.lookAt(cam_pos, glm.vec3(0,0,0), glm.vec3(0,1,0))
        # draw world frame
